# 🚀 Despliegue en EasyPanel - NaturaCare
...
psql -h naturacare-db -U postgres -d naturacare < database.sql

# Opción 2: Desde un cliente local
psql -h TU_HOST_EASYPANEL -U postgres -d naturacare -f database.sql
```
...
1. Construye y sube la imagen:
   ```bash
   docker build -t tu-usuario/naturacare:latest .
   docker push tu-usuario/naturacare:latest
   ```

2. En EasyPanel:
   - Add Service → App → From Docker Image
   - Image: `tu-usuario/naturacare:latest`
...
```bash
psql -h naturacare-db -U postgres -d naturacare
```
