# Clinica Base - Configuración de Base de Datos PostgreSQL
...
```bash
psql -U postgres -d clinic_base -f database.sql
```

**Opción B - Desde pgAdmin:**
1. Abre pgAdmin
2. Conecta a tu servidor PostgreSQL
3. Selecciona la base de datos `clinic_base`
...
```bash
pg_dump -U postgres clinic_base > backup_clinic_base.sql
```

### Restaurar backup:
```bash
psql -U postgres clinic_base < backup_clinic_base.sql
```
