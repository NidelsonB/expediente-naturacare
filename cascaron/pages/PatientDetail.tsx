...
const PatientDetail: React.FC<PatientDetailProps> = ({ patients, visits, addVisit, updatePatient, updateVisit, doctorName }) => {
  ...
  const doctorDisplayName = 'Dr(a). Responsable';
  ...
  const handlePrint = (visit: Visit) => {
    const printContent = `
      <div style="font-family: 'Inter', sans-serif; width: 210mm; min-height: 297mm; margin: 0 auto; background: #ffffff; color: #1f2937; padding: 18mm 16mm; box-sizing: border-box; display: flex; flex-direction: column;">
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10mm;">
          <div>
            <h1 style="margin: 0; font-size: 54px; line-height: 1; font-weight: 900; color: #10b981; letter-spacing: -1.8px;">Clinica Base</h1>
            <p style="margin: 7px 0 0; color: #64748b; font-weight: 800; font-size: 11px; text-transform: uppercase; letter-spacing: 1.4px;">Especialidades configurables</p>
            <p style="margin: 2px 0 0; color: #64748b; font-weight: 800; font-size: 11px; text-transform: uppercase; letter-spacing: 1.4px;">Atencion medica integral</p>
            <p style="margin: 4px 0 0; color: #0ea5a1; font-size: 11px; font-weight: 900; text-transform: uppercase; letter-spacing: 1.4px;">Tel: 0000-0000</p>
            <p style="margin: 2px 0 0; color: #94a3b8; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.7px;">Direccion comercial configurable</p>
          </div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-end; margin-top: auto;">
          ...
          <div style="width: 45%; text-align: center;">
            ...
            <p style="margin: 2px 0 0; font-size: 10px; color: #94a3b8; font-weight: 800; text-transform: uppercase; letter-spacing: 1px;">${escapeHtml(doctorDisplayName)}<br/>Clinica Base</p>
          </div>
        </div>
      </div>
    `;
    ...
    if (printWindow) {
      printWindow.document.write(`
        <html>
          <head>
            <title>Clinica Base_Receta_${patient.name.replace(/\s+/g, '_')}</title>
  ...
  const handlePrintCertificate = () => {
    ...
    if (printWindow) {
      printWindow.document.write(`
        <html>
          <head>
            <title>Clinica Base_Constancia_${patient.name.replace(/\s+/g, '_')}</title>
            <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap" rel="stylesheet">
            <style>
              body { margin: 0; font-family: 'Inter', sans-serif; background: #f1f5f9; }
              @page { size: A4; margin: 0; }
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-3">
              <div className="flex items-center space-x-3 text-emerald-100 text-[10px] font-black uppercase tracking-[0.2em]">
                ...
                <span className="text-white">Expediente Clinica Base</span>
              </div>
      ...
      {isNewVisitOpen && (
        <div ref={visitSectionRef} className="bg-white rounded-[2rem] shadow-2xl border-4 border-emerald-500 overflow-hidden animate-slideUp">
          <div className="bg-emerald-50 px-10 py-6 border-b border-emerald-100 flex justify-between items-center">
            <h3 className="text-xl font-black text-emerald-800 uppercase tracking-widest">Nueva Hoja Clinica Base</h3>
            <button 
              onClick={() => {
                setVisitNotes(['']);
                setVisitTreatment('');
              ...
              <div className="bg-slate-100 border border-slate-200 rounded-3xl p-5 md:p-7">
                <div className="mx-auto bg-white w-full max-w-[760px] min-h-[980px] p-8 md:p-10 border border-slate-200 shadow-sm flex flex-col">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <h5 className="text-4xl font-black text-emerald-500 tracking-tight leading-none">Clinica Base</h5>
                      <p className="mt-2 text-[11px] text-slate-500 font-black uppercase tracking-[0.18em]">Especialidades configurables</p>
                      <p className="text-[11px] text-slate-500 font-black uppercase tracking-[0.18em]">Atencion medica integral</p>
                      <p className="mt-1 text-[11px] text-teal-600 font-black uppercase tracking-[0.18em]">Tel: 0000-0000</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.1em]">Direccion comercial configurable</p>
                    </div>
                  ...
                  <div className="mt-auto pt-10 flex items-end justify-between gap-10">
                    ...
                    <div className="w-[42%] text-center">
                      ...
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.16em]">{doctorDisplayName}<br />Clinica Base</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="p-8 grid grid-cols-1 xl:grid-cols-2 gap-8">
          ...
          <div className="bg-slate-100 border border-slate-200 rounded-3xl p-5 md:p-7">
            <div className="mx-auto bg-white w-full max-w-[760px] min-h-[980px] p-8 md:p-10 border border-slate-200 shadow-sm flex flex-col">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h4 className="text-4xl font-black text-emerald-500 tracking-tight leading-none">Clinica Base</h4>
                  <p className="mt-2 text-[11px] text-slate-500 font-black uppercase tracking-[0.18em]">Especialidades configurables</p>
                  <p className="text-[11px] text-slate-500 font-black uppercase tracking-[0.18em]">Atencion medica integral</p>
                  <p className="mt-1 text-[11px] text-teal-600 font-black uppercase tracking-[0.18em]">Tel: 0000-0000</p>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.1em]">Direccion comercial configurable</p>
