    ...
    <div className="min-h-screen flex flex-col bg-slate-50 print:bg-white">
      <header className="bg-white border-b border-slate-200 sticky top-0 z-10 shadow-sm print:hidden">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <Link to="/" className="flex items-center space-x-2 group">
            ...
            <span className="font-extrabold text-slate-800 text-xl tracking-tight">Clinica Base</span>
          </Link>
          <div className="flex items-center space-x-6">
            <div className="hidden sm:block text-right">
              <p className="text-sm font-bold text-slate-900">Dr(a). {user.name}</p>
              <p className="text-xs text-emerald-600 font-semibold uppercase tracking-widest">Clinica Base</p>
            </div>
            <button 
              onClick={onLogout}
      ...
      <footer className="bg-white border-t border-slate-200 py-6 mt-auto print:hidden">
        <div className="max-w-6xl mx-auto px-4 text-center text-slate-400 text-sm font-medium">
          &copy; {new Date().getFullYear()} Clinica Base - Dr(a). Responsable. Sistema de Gestión Médica.
        </div>
      </footer>
    </div>
  );
...
const App: React.FC = () => {
  const [user, setUser] = useState<User | null>(() => {
    const saved = localStorage.getItem('whitelabel_user');
  ...
  const handleLogin = (newUser: User) => {
    setUser(newUser);
    localStorage.setItem('whitelabel_user', JSON.stringify(newUser));
  };

  const handleLogout = () => {
    setUser(null);
    localStorage.removeItem('whitelabel_user');
  };
